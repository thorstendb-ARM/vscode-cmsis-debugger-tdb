...
describe('ComponentViewerTargetAccess', () => {
    ...
    beforeEach(() => {
        ...
        targetAccess = new ComponentViewerTargetAccess(gdbTargetSession);
    });
    ...
    describe('isMemberInStruct', () => {
        it('should return true when member exists in struct', async () => {
            const structDefinition = `type = struct MyStruct {
                int field1;
                char field2;
                float field3;
            }`;
            ...
            const result = await targetAccess.isMemberInStruct('myStruct', 'field2');

            expect(result).toBe(true);
            expect(debugSession.customRequest).toHaveBeenCalledWith('evaluate', {
                expression: 'ptype myStruct',
                frameId: 2,
                context: 'hover'
            });
        ...
        it('should return false when member does not exist in struct', async () => {
            const structDefinition = `type = struct MyStruct {
                int field1;
                char field2;
                float field3;
            ...
            const result = await targetAccess.isMemberInStruct('myStruct', 'nonExistentField');
        ...
        it('should use frameId 0 when no active stack frame exists', async () => {
            const structDefinition = 'type = struct MyStruct { int member; }';
            (debugSession.customRequest as jest.Mock).mockResolvedValueOnce({ 
                result: structDefinition,
                variablesReference: 0 
            });
            ...
            const result = await targetAccess.isMemberInStruct('myStruct', 'member');
        ...
        it('should return false when evaluation fails', async () => {
            const logDebugSpy = jest.spyOn(logger, 'debug');
            (debugSession.customRequest as jest.Mock).mockRejectedValueOnce(new Error('Struct not found'));

            const result = await targetAccess.isMemberInStruct('unknownStruct', 'field');

            expect(result).toBe(false);
            expect(logDebugSpy).toHaveBeenCalledWith(
                "Session 'test-session': Failed to know if member field is part of 'unknownStruct' - 'Struct not found'"
            );
        });

        it('should return false when custom request fails', async () => {
            const logDebugSpy = jest.spyOn(logger, 'debug');
            (debugSession.customRequest as jest.Mock).mockRejectedValueOnce(new Error('custom request failed'));

            const result = await targetAccess.isMemberInStruct('myStruct', 'field');

            expect(result).toBe(false);
            expect(logDebugSpy).toHaveBeenCalledWith(
                "Session 'test-session': Failed to know if member field is part of 'myStruct' - 'custom request failed'"
            );
        });
    ...
    describe('constructor', () => {
        ...
        it('should create instance with different session', () => {
            ...
            const anotherTargetAccess = new ComponentViewerTargetAccess(anotherGDBSession);
